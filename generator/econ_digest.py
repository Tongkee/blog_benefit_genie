# -*- coding: utf-8 -*-
"""실험B: 유명 경제 블로그 소싱 → 베네핏지니화 재작성 (2026-07-29 사용자 지시).
...
def rhythm_issues(body: str) -> list[str]:
    ...
    근거(원본 대조 실측): 메르 원본은 117줄·평균 17자·최장 87자인데, 베네핏지니 톤으로
    ...
    head = lines[0]
    for bad in ("안녕하세요", "베네핏지니", "정리해 드릴", "정리해드릴", "드릴게요"):
        if bad in body:
...
def write_post(facts: dict, author: str, api_key: str, avoid: list[str] | None = None,
    ...
    prompt = (
        ...
        f"[추출 정보]\n{facts_json}\n\n"
        "★문체 — 이 코너는 '베네핏지니' 페르소나를 쓰지 않는다(2026-07-31 사용자 지시).\n"
        "1. 담백한 서술체로 쓴다('~다', '~였다', '~이다'). 인사말·자기소개·맺음인사 전부 금지"
        "('안녕하세요', '베네핏지니입니다', '~해 드릴게요', '~정리해 드릴게요').\n"
