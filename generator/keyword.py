"""
포스팅 키워드 선정
- 카테고리 기반 요일별 로테이션
- 에버그린 키워드 풀 + 시즌 보정
- 최근 30일 사용 키워드 중복 방지
"""
import glob
import json
import logging
import os
import random
import re
from datetime import datetime, timezone, timedelta
from xml.etree import ElementTree

import requests

logger = logging.getLogger(__name__)
KST = timezone(timedelta(hours=9))

# 카테고리 정의 — 요일별 포스팅 타겟 (0=월, 1=화, ..., 6=일)
CATEGORIES: dict[str, dict] = {
    "신혼살림기초": {
        "name": "신혼살림 기초",
        "keywords": [
            "신혼 살림 체크리스트",
            "신혼 필수템 목록",
            "혼수 준비 리스트",
            "신혼집 장만 순서",
            "신혼 살림 예산 짜는 법",
            "결혼 준비 살림 뭐부터",
            "신혼집 첫 살림 필수품",
            "혼수 적게 쓰는 방법",
            "신혼 살림 다이소로 완성",
            "신혼 이불 세탁 주기",
        ],
        "posting_days": [1, 4],  # 화, 금
    },
    "청소정리": {
        "name": "청소&정리",
        "keywords": [
            "욕실 청소 꿀팁",
            "주방 기름때 제거",
            "세탁기 청소 주기",
            "욕실 곰팡이 제거",
            "청소 루틴 만들기",
            "환기구 청소 방법",
            "싱크대 배수구 냄새 제거",
            "베란다 청소 꿀팁",
            "샤워실 물때 제거",
            "냉장고 청소 방법",
            "에어컨 필터 청소",
            "공기청정기 필터 교체 주기",
        ],
        "posting_days": [2, 5],  # 수, 토
    },
    "요리식비": {
        "name": "요리&식비절약",
        "keywords": [
            "냉파 요리 아이디어",
            "식비 절약 일주일 식단",
            "간단한 아침 메뉴",
            "자취 요리 초보 레시피",
            "일주일 반찬 만들기",
            "냉동식품 활용 요리",
            "식비 한 달 20만원",
            "간단 도시락 메뉴",
            "주말 요리 일주일 치",
            "칼로리 낮은 간단 요리",
        ],
        "posting_days": [3, 6],  # 목, 일
    },
    "절약재테크": {
        "name": "절약&재테크",
        "keywords": [
            "생활비 절약 방법",
            "관리비 줄이는 법",
            "전기세 절약 방법",
            "가계부 쓰는 법",
            "신혼 생활비 얼마",
            "체크카드 vs 신용카드",
            "생활비 통장 쪼개기",
            "공과금 자동이체 혜택",
            "알뜰폰으로 바꾸기",
            "구독서비스 정리하기",
            "난방비 절약 꿀팁",
            "냉방비 절약 방법",
        ],
        "posting_days": [1, 3],  # 월(0), 화(1), 수(2) -> 화(1), 목(3)
    },
    "인테리어": {
        "name": "인테리어&소품",
        "keywords": [
            "자취 인테리어 소품",
            "원룸 꾸미기 꿀팁",
            "다이소 인테리어 소품",
            "신혼집 인테리어 저렴하게",
            "옷장 정리 방법",
            "주방 수납 꿀팁",
            "베란다 정리 수납",
            "소형 냉장고 정리법",
            "무드등 추천",
            "화장대 정리 방법",
            "이케아 수납 아이디어",
            "드레스룸 없는 집 옷 정리",
        ],
        "posting_days": [2, 5],  # 수, 토
    },
    "쇼핑정보": {
        "name": "쇼핑&제품추천",
        "keywords": [
            "다이소 추천템",
            "이케아 추천 제품",
            "주방용품 추천",
            "신혼 살림 추천 제품",
            "다이소 청소 용품",
            "쿠팡 생활용품 추천",
            "이케아 수납 추천",
            "로봇청소기 추천",
            "스팀 청소기 추천",
            "공기청정기 추천",
            "전기밥솥 추천",
            "식기세척기 추천",
        ],
        "posting_days": [0, 4],  # 월, 금
    },
    "신혼일상": {
        "name": "신혼일상",
        "keywords": [
            "신혼일기 주말 데이트 요리",
            "퇴근 후 남편이랑 저녁 차려먹기",
            "주말에 신혼집 대청소한 날",
            "다이소 쇼핑 2만원의 행복",
            "남편이랑 가계부 점검일기",
            "여름맞이 이불 세탁하고 코인빨래방 다녀옴",
            "신혼부부 식비 절약 냉장고 털기 일상",
            "초보 주부의 하루 살림 일기",
            "수원 신혼 아파트 첫 집들이 날",
            "주말 이케아 다녀온 후기",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },
    "제품리뷰": {
        "name": "제품리뷰",
        "keywords": [
            "다이소 청소솔 솔직 후기",
            "쿠팡 물걸레 청소기 가성비 후기",
            "이케아 수납바구니 써본 후기",
            "다이소 욕실 정리함 솔직 후기",
            "쿠팡 에어프라이어 가성비 비교",
            "다이소 폼클렌저 거품 성능 비교",
            "이케아 빌리 책장 조립 후기",
            "다이소 세탁세제 효과 테스트",
            "쿠팡 싱크대 세제 사용 후기",
            "다이소 냉장고 탈취제 효과 후기",
            "쿠팡 스텐냄비 가성비 후기",
            "무인양품 수건 세탁 후기",
            "다이소 가습기 솔직 후기",
            "쿠팡 공기청정기 가성비 후기",
        ],
        "posting_days": [0, 2, 4],  # 월, 수, 금
    },
    "정보성": {
        "name": "생활정보",
        "keywords": [
            "청년희망적금 신청 방법",
            "신혼부부 주택청약 조건",
            "고유가지원금 신청 대상",
            "아이폰 17 국내 출시일",
            "청년내일저축계좌 신청 기간",
            "전세보증보험 가입 방법",
            "갤럭시 S26 가격 출시일",
            "청년 월세 지원 신청 방법",
            "에어컨 전기세 계산법",
            "신혼부부 대출 종류 정리",
            "청약 가점 계산 방법",
            "전입신고 인터넷으로 하는 법",
        ],
        "posting_days": [1, 3, 5],  # 화, 목, 토
    },
    # ── 건강 카테고리 (health_post.py 전용) ──────────────────────────────
    "건강식품": {
        "name": "건강식품&영양",
        "keywords": [
            "혈관 건강에 좋은 음식",
            "치매 예방 음식 5가지",
            "고지혈증에 좋은 음식",
            "혈당 낮추는 식품",
            "항산화 음식 추천",
            "면역력 높이는 음식",
            "피로회복에 좋은 음식",
            "장 건강에 좋은 음식",
            "콜레스테롤 낮추는 식품",
            "뼈에 좋은 음식",
            "눈 건강에 좋은 음식",
            "간 건강에 좋은 음식",
            "신장에 좋은 음식",
            "혈압 낮추는 음식",
            "당뇨에 좋은 음식",
            "심장에 좋은 음식",
            "위장에 좋은 음식",
            "변비에 좋은 음식",
            "빈혈에 좋은 음식",
            "갑상선에 좋은 음식",
            "오메가3 효과 제대로 보는 법",
            "마그네슘 부족 증상",
            "비타민D 부족 증상",
            "아연 효능 및 복용법",
            "유산균 효능 먹는 시간",
            "콜라겐 효능 흡수율",
            "코엔자임큐텐 효능",
            "루테인 효능 눈 건강",
            "밀크시슬 효능 간 건강",
            "홍삼 효능 부작용",
            "40대부터 꼭 먹어야 할 영양소",
            "근육 지키는 단백질 음식",
            "저속노화 식단 방법",
            "만성염증 줄이는 음식",
            "혈관을 청소하는 음식",
            "뇌 건강에 좋은 음식",
            "수면에 좋은 음식",
            "스트레스 낮추는 음식",
            "피부 좋아지는 음식",
            "탈모에 좋은 음식",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },
    "다이어트": {
        "name": "다이어트&체중관리",
        "keywords": [
            "마운자로 효과 없는 이유",
            "마운자로 부작용 주의사항",
            "마운자로 vs 오젬픽 차이",
            "GLP-1 다이어트 약 종류",
            "삭센다 효과 사용법",
            "위고비 국내 출시",
            "다이어트 정체기 탈출 방법",
            "아랫배 살 빠지는 식단",
            "뱃살 빠지는 음식",
            "기초대사량 높이는 방법",
            "공복 혈당 낮추는 방법",
            "혈당 스파이크 줄이는 식단",
            "간헐적 단식 방법 16 8",
            "저탄고지 식단 방법",
            "지중해식 식단 효과",
            "체중감량 점심 식단",
            "다이어트 아침 식단",
            "저칼로리 포만감 음식",
            "다이어트 중 근손실 막는 법",
            "굶어도 살 안 빠지는 이유",
            "나잇살 빠지는 방법",
            "복부비만 원인과 해결책",
            "내장지방 빠지는 운동",
            "유산소 vs 근력 다이어트",
            "다이어트 식이섬유 효과",
            "단백질 다이어트 방법",
            "다이어트 중 외식 메뉴",
            "야식 끊는 방법",
            "폭식 후 회복 식단",
            "다이어트 리바운드 방지",
            "체지방 줄이는 음식",
            "비만 원인 의외의 이유",
            "살 안 찌는 체질 만드는 법",
            "체중계 숫자 안 줄어드는 이유",
            "다이어트 음식 잘못된 상식",
            "요요 없이 살 빼는 방법",
            "다이어트 방해하는 음식",
            "운동 없이 살 빼는 방법",
            "2주 만에 3kg 빼는 식단",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },
    "정부지원혜택": {
        "name": "정부지원&혜택",
        "keywords": [
            # 청년/신혼 지원
            "신혼부부 전세대출 조건 2026",
            "청년 주거급여 신청 방법",
            "신혼부부 디딤돌대출 조건",
            "청년도약계좌 신청 방법",
            "청년희망저축계좌 가입 조건",
            "신혼부부 주택청약 1순위 조건",
            "청년 월세 한시 특별지원 신청",
            "신혼부부 취득세 감면 조건",
            "출산장려금 신청 방법",
            "첫만남이용권 사용 방법",
            "부모급여 신청 대상 금액",
            "아동수당 신청 방법",
            "육아휴직급여 계산 방법",
            "출산휴가 급여 신청 방법",
            "임신 출산 바우처 국민행복카드",
            # 에너지/생활비 지원
            "에너지바우처 신청 대상 방법",
            "에너지캐시백 신청 방법",
            "취약계층 에너지 지원 신청",
            "도시가스 절약 캐시백",
            "전기요금 복지할인 신청",
            "주거급여 신청 자격 방법",
            # 건강/의료 지원
            "국가건강검진 대상 항목 2026",
            "청년 마음건강 바우처 신청",
            "의료급여 신청 방법",
            "본인부담상한제 환급 방법",
            "암검진 무료 대상 신청",
            "치매안심센터 서비스",
            # 취업/교육 지원
            "청년내일채움공제 신청",
            "내일배움카드 신청 방법",
            "국민취업지원제도 신청",
            "청년 일경험 지원사업",
            "근로장려금 신청 방법",
            "자녀장려금 신청 조건",
            # 복지/생활 지원
            "긴급복지지원 신청 방법",
            "기초생활수급자 신청 조건",
            "차상위계층 혜택 정리",
            "한부모가족 지원금 신청",
            "장애인 복지 혜택 정리",
            "노인 기초연금 신청 방법",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },
    "생활건강": {
        "name": "생활건강&증상",
        "keywords": [
            "아무리 자도 피곤한 이유",
            "만성피로 원인과 해결책",
            "만성염증 증상 확인법",
            "수면 부족 몸에 나타나는 증상",
            "잠을 잘 자는 방법",
            "불면증 해결 방법",
            "하루 몇 시간 자야 하는지",
            "두통 원인 의외의 이유",
            "편두통 유발 음식",
            "소화 안 될 때 빠른 해결법",
            "역류성 식도염 좋은 음식",
            "위염에 좋은 음식",
            "과민성대장증후군 식단",
            "변비 빨리 해결하는 방법",
            "혈압 낮추는 생활 습관",
            "혈당 관리 생활 습관",
            "당뇨 전단계 식단",
            "콜레스테롤 낮추는 생활 습관",
            "고지혈증 식단 관리",
            "갱년기 증상 완화 방법",
            "여성 호르몬 부족 증상",
            "생리통 심할 때 해결법",
            "탈모 원인 여성 탈모",
            "피부 트러블 원인 음식",
            "여드름 없애는 방법",
            "피부 건조할 때 관리법",
            "눈 충혈 원인 해결법",
            "눈 떨림 원인 마그네슘",
            "손발 저림 원인",
            "관절 통증 줄이는 방법",
            "허리 통증 원인 생활 습관",
            "목 어깨 통증 원인",
            "면역력 낮을 때 나타나는 증상",
            "감기 빨리 낫는 방법",
            "비타민D 부족 증상 확인",
            "빈혈 증상 확인법",
            "갑상선 기능 저하 증상",
            "당뇨 초기 증상 확인",
            "고혈압 초기 증상",
            "암 초기 증상 놓치기 쉬운",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },

    # ── 금융·재테크 (고CPC 정보성) ──────────────────────────────
    "금융재테크": {
        "name": "금융&재테크",
        "keywords": [
            "청년도약계좌 만기 수령액 계산",
            "예금 적금 금리 비교 방법",
            "주택청약통장 납입 인정 금액",
            "ISA 계좌 장점 단점 비교",
            "연금저축 세액공제 한도",
            "IRP 계좌 장단점 수수료",
            "신용점수 올리는 방법",
            "마이너스 통장 금리 조건",
            "비상금 통장 파킹통장 추천",
            "대출 갈아타기 대환대출 방법",
            "신용대출 한도 늘리는 법",
            "전세보증금 반환보증 가입",
            "카드 포인트 현금화 방법",
            "체크카드 신용카드 차이 혜택",
            "주식 배당금 받는 방법 기초",
            "CMA 통장 이자 비교",
            "적금 풍차돌리기 방법",
            "환테크 달러 투자 방법",
            "금 투자 방법 KRX 금시장",
            "퇴직연금 DC형 DB형 차이",
            "예금자보호 한도 1억 확인",
            "파킹통장 금리 비교 방법",
            "청년미래적금 가입 조건",
            "주택담보대출 금리 비교",
            "전세자금대출 한도 조건",
            "중도상환수수료 면제 조건",
            "신용카드 연회비 아끼는 법",
            "비과세 종합저축 가입 조건",
            "채권 투자 시작하는 법",
            "ETF 적립식 투자 방법",
            "연금저축 IRP 차이 비교",
            "퇴직금 IRP 이체 방법",
            "국민연금 추납 제도 활용",
            "노란우산공제 소득공제",
            "저축은행 예금 안전성 확인",
            "달러 예금 환율 우대 방법",
            "자동차 할부 대출 비교",
            "통장 쪼개기 관리 방법",
            "비상금 적정 금액 기준",
            "신용점수 무료 조회 방법",
            "대출 중도상환 순서 전략",
            "주택청약 예치금 기준 금액",
            "적금 만기 재예치 전략",
            "카드 리볼빙 위험성",
            "금리인하요구권 신청 방법",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },

    # ── 세금·절세 (고CPC, 시즌 검색폭발) ──────────────────────────────
    "세금절세": {
        "name": "세금&절세",
        "keywords": [
            "연말정산 환급 많이 받는 법",
            "연말정산 인적공제 조건",
            "월세 세액공제 신청 방법",
            "종합소득세 신고 대상 기준",
            "프리랜서 종합소득세 신고 방법",
            "부가가치세 신고 방법 간이과세",
            "자녀 세액공제 조건 금액",
            "의료비 세액공제 한도",
            "교육비 세액공제 대상",
            "주택청약 소득공제 조건",
            "신용카드 소득공제 한도 계산",
            "기부금 세액공제 방법",
            "양도소득세 비과세 요건",
            "증여세 면제 한도 자녀",
            "상속세 면제 한도 기준",
            "퇴직소득세 계산 방법",
            "사업자 경비 처리 절세 방법",
            "주택임대소득 신고 방법",
            "연말정산 미리보기 활용법",
            "월세 세액공제 조건 서류",
            "교육비 세액공제 대상 범위",
            "기부금 세액공제 종류",
            "신용카드 소득공제 한도",
            "부양가족 인적공제 요건",
            "맞벌이 부부 공제 전략",
            "중소기업 취업자 소득세 감면",
            "프리랜서 원천징수 환급 방법",
            "부가가치세 신고 방법",
            "간이과세자 일반과세자 차이",
            "종합소득세 신고 대상 확인",
            "자동차세 연납 할인 신청",
            "지방소득세 신고 방법",
            "세금 환급금 조회 방법",
            "경정청구 기한과 방법",
            "현금영수증 의무발행 업종",
            "기타소득 필요경비 계산",
            "연금소득세 과세 기준",
            "종합소득세 분리과세 선택",
            "세금 체납 불이익 정리",
            "홈택스 전자세금계산서 발행",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },

    # ── 보험 (고CPC, 건강↔돈 브릿지) ──────────────────────────────
    # 2026-07-06 재편: 국민연금(공적연금, 주제표류) 제거 + 20→42개 확장
    # (시즌형은 DataLab 트렌딩이 검색량으로 제철에 알아서 우선 선택)
    "보험": {
        "name": "보험",
        "keywords": [
            # 실손·건강
            "실손보험 청구 방법 서류",
            "4세대 실손보험 전환 장단점",
            "실비보험 중복 가입 보장",
            "실비보험 비급여 보장 범위",
            "도수치료 실손보험 보장 조건",
            "단체 실손보험 개인 실손 중지제도",
            "실손보험 세대별 차이 비교",
            "암보험 가입 전 체크 사항",
            "암 진단비 얼마가 적당",
            "유사암 일반암 보장 차이",
            "뇌 심장 2대 진단비 보험",
            "치아보험 보장 내용 비교",
            "간병보험 치매보험 차이",
            "유병자 보험 가입 조건",
            "건강보험 피부양자 자격 조건",
            # 자동차·운전자
            "자동차보험 저렴하게 가입하는 법",
            "다이렉트 자동차보험 비교",
            "자동차보험 비교 플랫폼 이용법",
            "자동차보험 갱신 보험료 인상 대처",
            "자동차보험 할인 특약 총정리",
            "운전자보험 꼭 필요한 특약",
            "운전자보험 변호사비용 벌금 한도",
            "차량 침수 피해 자동차보험 보상",
            "1인 신차 자동차보험 첫 가입",
            # 연금·저축성
            "연금보험 연금저축 차이",
            "정기보험 종신보험 차이",
            "저해지 무해지 보험 차이",
            "종신보험 해지 손해 판단",
            "보험 해지 대신 감액완납",
            "보험 해지 환급금 계산",
            # 생활·배상·기타
            "일상생활배상책임보험 활용법",
            "전세 세입자 화재보험 필요성",
            "해외여행자보험 가입 비교",
            "여행자보험 휴대품 손해 보장",
            "펫보험 가입 조건 보장 비교",
            "태아보험 가입 시기 보장",
            "어린이보험 필수 특약",
            # 청구·관리·리모델링
            "보험금 청구 거절 대처법",
            "숨은 보험금 찾기 내보험찾아줌",
            "보험금 청구 소멸시효 기간",
            "보험 리모델링 방법",
            "신혼부부 보험 리모델링 순서",
            "30대 보험 포트폴리오 월 보험료",
            "사회초년생 첫 보험 우선순위",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },

    # ── 부동산·주거 (고CPC, 정부지원과 직결) ──────────────────────────────
    "부동산주거": {
        "name": "부동산&주거",
        "keywords": [
            "주택청약 1순위 조건 가점 계산",
            "특별공급 신혼부부 조건",
            "생애최초 특별공급 자격",
            "전세자금대출 조건 한도",
            "버팀목 전세자금대출 자격",
            "디딤돌대출 조건 금리",
            "보금자리론 자격 한도",
            "전세 보증금 지키는 방법",
            "전세사기 예방 체크리스트",
            "확정일자 전입신고 방법",
            "임대차계약 주의사항",
            "전월세 신고제 대상 방법",
            "청년 전세임대주택 신청",
            "행복주택 입주 자격 조건",
            "LH 국민임대 신청 방법",
            "부동산 취득세 계산 방법",
            "재산세 부과 기준 납부",
            "전세 월세 전환율 계산",
            "등기부등본 보는 법 확인",
            "계약갱신청구권 사용 방법",
            "임대차 3법 핵심 정리",
            "보증금 증액 상한 기준",
            "전세권 설정 확정일자 차이",
            "임차권등기명령 신청 방법",
            "부동산 중개보수 요율",
            "등기부등본 확인 필수 항목",
            "아파트 관리비 절약 방법",
            "장기수선충당금 반환 청구",
            "신혼희망타운 자격 조건",
            "청년 매입임대주택 신청",
            "공공임대 소득 자산 기준",
            "재개발 재건축 차이점",
            "분양권 전매제한 기간",
            "특별공급 종류와 자격",
            "무순위 청약 신청 조건",
            "주택연금 가입 조건 수령액",
            "일시적 2주택 비과세 요건",
            "오피스텔 주택수 포함 여부",
            "상가 임대차보호법 핵심",
            "하자보수 청구 절차",
            "전입신고 세대주 변경",
            "다가구 다세대 차이",
            "깡통전세 확인 방법",
            "이사 전 체크리스트 정리",
        ],
        "posting_days": [0, 1, 2, 3, 4, 5, 6],
    },
}

# 시즌 키워드 (월별 보정)
_SEASON: dict[int, list[str]] = {
    1:  ["겨울 난방비 절약", "신년 집 정리", "방한 꿀템"],
    2:  ["봄맞이 대청소 준비", "설날 집 준비", "봄 인테리어 미리 준비"],
    3:  ["봄 대청소 루틴", "황사 미세먼지 대비", "봄옷 정리 수납"],
    4:  ["봄 인테리어 소품", "꽃가루 알레르기 집 관리", "창문 청소 방법"],
    5:  ["가정의 달 선물 추천", "초여름 집 꾸미기", "에어컨 청소 시기"],
    6:  ["여름 에어컨 관리", "냉방비 절약", "여름 냄새 제거"],
    7:  ["여름 집 습기 관리", "여름 곰팡이 예방", "여름 정리 정돈"],
    8:  ["피서 대신 집꾸미기", "여름 인테리어", "선풍기 청소 방법"],
    9:  ["가을 이불 교체", "환절기 집 관리", "김장 준비 용품"],
    10: ["가을 인테리어", "단열 점검 방법", "겨울 준비 시작"],
    11: ["김장 도구 추천", "겨울 이불 수납", "난방기기 점검"],
    12: ["연말 집 정리", "크리스마스 홈 데코", "겨울 살림 필수템"],
}

_HISTORY_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "data", "post_history.json"
)

# 주제 클러스터 — 같은 계열 키워드가 짧은 간격으로 연속 발행되는 것 방지
# (2026-07-06 실사고: 보험 자동차보험 2연속 — 키워드가 서로 달라 30일 회피를 통과함.
#  클러스터 미매칭 키워드는 제한 없음. 현재 보험만, 필요 시 카테고리 추가.)
TOPIC_CLUSTERS: dict[str, dict[str, list[str]]] = {
    "보험": {
        "실손": ["실손", "실비", "도수치료", "비급여"],
        "자동차": ["자동차보험", "운전자보험", "침수", "신차"],
        "진단비": ["암", "진단비", "유사암", "뇌 심장"],
        "연금저축성": ["연금", "종신", "정기보험", "저해지", "무해지", "해지", "감액완납"],
        "생활배상": ["일상생활배상", "화재보험", "여행자보험", "휴대품", "펫보험"],
        "가족": ["태아보험", "어린이보험"],
        "청구관리": ["청구", "숨은 보험금", "내보험찾아줌", "소멸시효", "리모델링", "포트폴리오", "첫 보험"],
    },
}


def keyword_cluster(cat_id: str, keyword: str) -> str | None:
    """키워드가 속한 주제 클러스터 반환 (미매칭이면 None = 제한 없음)."""
    for cluster, subs in TOPIC_CLUSTERS.get(cat_id, {}).items():
        if any(s in keyword for s in subs):
            return cluster
    return None


_DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data")
# 베네핏지니 블로그에 발행되는 모든 정보성 트랙의 이력
_BLOG_TRACK_HISTORIES = ("info_*_history.json", "gov_history.json")


def recent_keywords_all_tracks(days: int = 30) -> set[str]:
    """베네핏지니 블로그 **전 트랙**의 최근 N일 발행 키워드.

    ★2026-07-31 실사고: 중복 검사가 자기 카테고리 이력 파일만 봐서, 같은 키워드가
    다른 카테고리로 연달아 나갔다('여름 집 습기 관리' 7/30 부동산·주거 → 7/31 금융·재테크,
    '자녀장려금' 정부지원 → 세금·절세 2회). 독자와 검색엔진에는 한 블로그이므로
    중복 판정도 블로그 단위여야 한다.
    """
    cutoff = (datetime.now(KST) - timedelta(days=days)).strftime("%Y-%m-%d")
    out: set[str] = set()
    for pat in _BLOG_TRACK_HISTORIES:
        for path in glob.glob(os.path.join(_DATA_DIR, pat)):
            try:
                with open(path, encoding="utf-8") as f:
                    rows = json.load(f)
            except Exception:
                continue
            if not isinstance(rows, list):
                continue
            for h in rows:
                if not isinstance(h, dict):
                    continue
                if h.get("status") != "posted":
                    continue
                if str(h.get("date") or "") >= cutoff and h.get("keyword"):
                    out.add(h["keyword"])
    return out


def _get_recent_keywords(days: int = 30) -> set[str]:
    """최근 N일 이내 사용한 키워드 반환"""
    try:
        if not os.path.exists(_HISTORY_PATH):
            return set()
        with open(_HISTORY_PATH, encoding="utf-8") as f:
            history = json.load(f)
        cutoff = datetime.now(KST) - timedelta(days=days)
        recent = set()
        for h in history:
            try:
                ts = datetime.fromisoformat(h.get("timestamp", ""))
                if ts >= cutoff and h.get("keyword"):
                    recent.add(h["keyword"])
            except Exception:
                continue
        return recent
    except Exception as e:
        logger.warning(f"이력 키워드 로드 실패: {e}")
        return set()


def _get_today_category() -> tuple[str, dict]:
    """오늘 요일에 맞는 카테고리 선택. 복수 카테고리 해당 시 랜덤 선택."""
    weekday = datetime.now(KST).weekday()  # 0=월, 1=화, ..., 6=일
    matched = [
        (cat_id, cat_info)
        for cat_id, cat_info in CATEGORIES.items()
        if weekday in cat_info["posting_days"]
    ]
    if matched:
        chosen = random.choice(matched)
        return chosen
    # 해당 요일 카테고리 없으면 전체에서 랜덤
    cat_id = random.choice(list(CATEGORIES.keys()))
    return cat_id, CATEGORIES[cat_id]


def pick_keyword() -> dict:
    """
    오늘 포스팅할 키워드 1개 선택.
    반환: {"keyword": str, "category": str, "category_name": str}
    최근 30일 중복 방지 + 요일별 카테고리 로테이션
    """
    month = datetime.now(KST).month
    recent = _get_recent_keywords(days=30)

    # 오늘 카테고리 선정
    cat_id, cat_info = _get_today_category()
    cat_pool = cat_info["keywords"] + _SEASON.get(month, [])

    # 카테고리 내 미사용 키워드 우선
    fresh = [k for k in cat_pool if k not in recent]
    if not fresh:
        logger.warning(f"카테고리 {cat_info['name']} 키워드 소진 — 전체 풀에서 선택")
        # 전체 카테고리 키워드에서 찾기
        all_keywords = []
        for ci in CATEGORIES.values():
            all_keywords.extend(ci["keywords"])
        all_keywords.extend(_SEASON.get(month, []))
        fresh = [k for k in all_keywords if k not in recent]
        if not fresh:
            logger.warning("전체 키워드 소진 — 카테고리 풀에서 재사용")
            fresh = cat_pool

    keyword = random.choice(fresh)
    logger.info(
        f"키워드 선택: {keyword!r} | 카테고리: {cat_info['name']} "
        f"(신규 후보 {len(fresh)}/{len(cat_pool)}개)"
    )
    return {
        "keyword": keyword,
        "category": cat_id,
        "category_name": cat_info["name"],
    }


def get_trending_bonus(limit: int = 5) -> list[str]:
    """Google News RSS에서 생활 관련 트렌딩 키워드 보조 수집"""
    _SKIP = re.compile(
        r"북한|무기|전쟁|핵|테러|사망|사고|화재|범죄|살인|마약|자살|부고|피해|사건|정치|선거|"
        r"트럼프|바이든|푸틴|시진핑|총리|의원|여당|야당|검찰|기소|법원|판결|외교|국방|"
        r"대통령|국회|정당|총선|대선|장관|수석|참모|의혹|비리|부패|탄핵"
    )
    try:
        r = requests.get(
            "https://news.google.com/rss?hl=ko&gl=KR&ceid=KR:ko",
            timeout=8, headers={"User-Agent": "Mozilla/5.0"},
        )
        root = ElementTree.fromstring(r.content)
        topics = []
        for item in root.findall(".//item/title"):
            title = (item.text or "").strip()
            if not title or _SKIP.search(title):
                continue
            title = re.sub(r"\s+-\s+\S+$", "", title).strip()
            if len(title) > 4:
                topics.append(title)
            if len(topics) >= limit:
                break
        return topics
    except Exception as e:
        logger.warning(f"트렌딩 수집 실패: {e}")
        return []


# 블로그 카테고리 → 키워드 풀 매핑.
# ★2026-07-31 실사고: 카테고리명을 쉼표에서 가운뎃점으로 리네임(e47cbb3)했는데 이 표만
#   옛 이름('금융, 재테크')을 키로 갖고 있었다. 조회가 빗나가면 조용히 기본값 '청소정리'
#   (살림 청소 풀)로 떨어져서, 고CPC 4개 카테고리가 통째로 살림 키워드를 뽑았다.
#   결과: '베란다 청소, 최대 3,600만원 경비 처리로 세금 아끼는 법' 같은 없는 제도를
#   지어낸 글 4건 발행. 구분자 차이를 흡수하고, 못 찾으면 조용히 넘어가지 않는다.
_BLOG_CAT_TO_KEYWORD_CATS: dict[str, list[str]] = {
    "알뜰 살림 꿀팁": ["청소정리", "절약재테크", "신혼살림기초", "정보성"],
    "살림템 비교·추천": ["인테리어", "쇼핑정보", "제품리뷰"],
    "일상": ["신혼일상"],
    "건강·다이어트": ["건강식품", "다이어트", "생활건강"],
    "정부지원·혜택": ["정부지원혜택"],
    # 고CPC 정보성 (2026-06-30 피벗)
    "금융·재테크": ["금융재테크"],
    "세금·절세": ["세금절세"],
    "보험": ["보험"],
    "부동산·주거": ["부동산주거"],
}


class KeywordPoolExhausted(RuntimeError):
    """최근 30일 안에 카테고리 풀을 다 써서 새 키워드가 없음 — 중복 발행 대신 슬롯 스킵."""


def _norm_blog_cat(s: str) -> str:
    """카테고리명 정규화 — 구분자(쉼표·가운뎃점·슬래시)와 공백 차이를 흡수."""
    return re.sub(r"[\s,·・･、/]+", "", str(s or ""))


_BLOG_CAT_LOOKUP = {_norm_blog_cat(k): v for k, v in _BLOG_CAT_TO_KEYWORD_CATS.items()}


def blog_category_keyword_cats(blog_category: str) -> list[str]:
    """블로그 카테고리에 대응하는 키워드 풀 id 목록.

    ★못 찾으면 예외를 던진다. 종전처럼 살림 풀로 조용히 폴백하면, 재테크 카테고리에
    '욕실 곰팡이 제거'가 배정되고 그 위에 금리·가입조건 템플릿이 씌워진다.
    발행을 한 건 거르는 편이 엉뚱한 글을 내보내는 것보다 낫다.
    """
    cats = _BLOG_CAT_LOOKUP.get(_norm_blog_cat(blog_category))
    if not cats:
        raise ValueError(
            f"블로그 카테고리 {blog_category!r}에 대응하는 키워드 풀이 없습니다. "
            f"generator/keyword.py의 _BLOG_CAT_TO_KEYWORD_CATS에 추가하세요. "
            f"(등록된 이름: {sorted(_BLOG_CAT_TO_KEYWORD_CATS)})")
    return cats


def pick_keyword_for_blog_category(blog_category: str, exclude: set | None = None) -> dict:
    """
    특정 블로그 카테고리에 적합한 키워드를 선정하여 반환.
    최근 30일 중복 방지 + DataLab 트렌딩 우선 선택(30대 여성 기준).
    """
    keyword_cats = blog_category_keyword_cats(blog_category)
    chosen_cat_id = random.choice(keyword_cats)
    cat_info = CATEGORIES[chosen_cat_id]

    month = datetime.now(KST).month
    recent = _get_recent_keywords(days=30)
    # ★호출측(정보성/gov)의 자기 카테고리 이력 — _HISTORY_PATH(살림 post_history.json)엔
    # 정보성 발행이 안 남아 30일 회피가 뚫렸음(보험 '국민연금' 이틀 연속 등 실사고 2026-07-06).
    # 풀 필터 단계에서 제외해야 DataLab 트렌딩이 같은 키워드를 반복 반환하지 못한다.
    if exclude:
        recent = recent | set(exclude)

    cat_pool = cat_info["keywords"]
    if chosen_cat_id == "청소정리":
        cat_pool = cat_pool + _SEASON.get(month, [])

    # ★종료·폐지 제도는 주제로 뽑지 않는다(2026-07-31). 풀에 '청년도약계좌 만기 수령액
    # 계산'처럼 이미 신규가입이 끝난 제도가 남아 있었다. 생성 단계의 terminated_mentions
    # 게이트가 발행을 막아주긴 하지만, 애초에 뽑지 않아야 슬롯이 낭비되지 않는다.
    try:
        from generator.official_facts import discontinued_names
        dead = discontinued_names()
        drop = [k for k in cat_pool if any(d in k for d in dead)] if dead else []
        if drop:
            logger.info(f"종료 제도 키워드 제외: {drop}")
            cat_pool = [k for k in cat_pool if k not in drop]
    except Exception as e:
        logger.warning(f"종료 제도 목록 조회 실패(무시): {e.__class__.__name__}")

    fresh = [k for k in cat_pool if k not in recent]
    if not fresh:
        # ★fail-closed(2026-07-31). 종전엔 `fresh = cat_pool`로 풀 전체를 다시 열어
        # **30일 중복 게이트가 스스로 꺼졌다.** 실측(2026-07-31): 세금·절세 0/18,
        # 부동산·주거 1/19, 금융·재테크 2/20 — 오늘 아침 넣은 교차 중복 차단이
        # 이 폴백으로 이미 우회되고 있었다. 같은 키워드 재발행이 실제로 자기잠식을
        # 일으킨 증거도 있다(자녀장려금 275뷰 vs 재발행분 7뷰).
        # 중복글을 내느니 그 슬롯을 거른다 — 잃는 건 7뷰, 지키는 건 원글의 검색 순위.
        raise KeywordPoolExhausted(
            f"'{cat_info['name']}' 키워드 풀 소진 — 최근 30일에 {len(cat_pool)}개를 모두 사용했다. "
            f"중복 발행을 막기 위해 이 슬롯을 건너뛴다. 풀 보충이 필요하다.")

    # DataLab 검색어트렌드 우선 선택 (고CPC 정보성 카테고리만 — API 쿼터 절약)
    _DATALAB_CATS = {"금융재테크", "세금절세", "보험", "부동산주거", "정부지원혜택"}
    keyword = None
    if chosen_cat_id in _DATALAB_CATS and len(fresh) > 1:
        try:
            from generator.datalab_trend import pick_trending_keyword
            # 2026-07-19: [:20] 고정 절단 → 풀 전체(캡 40) — 리스트 뒤쪽 키워드가 검색량
            # 평가조차 못 받던 사각 제거(12h 캐시라 쿼터 부담 미미)
            keyword = pick_trending_keyword(fresh[:40], top_n=3)
        except Exception as e:
            logger.warning(f"DataLab 트렌딩 조회 실패 — 랜덤 선택: {e}")

    if keyword is None:
        keyword = random.choice(fresh)

    logger.info(
        f"블로그 카테고리 '{blog_category}' → 키워드: {keyword!r} | "
        f"키워드 카테고리: {cat_info['name']}"
    )
    return {
        "keyword": keyword,
        "category": chosen_cat_id,
        "category_name": cat_info["name"],
    }

